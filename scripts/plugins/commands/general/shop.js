...
const shop = (sender, page, category = undefined) => {
    if (!category) {
        ...
        .setTitle(`Shop Categories §f${page}§r/§f${pages}`)
        ...
        form.force(sender, (res) => {
            ...
            else if (res.selection == 48) shop(sender, page > 1 ? page - 1 : page)
            else if (res.selection == 50) shop(sender, page < pages ? page + 1 : page)
        ...
        const form = new CX.chestForm('large')
        .setTitle(`${category} §f${page}§r/§f${pages}`)
        .addButton(49, '§cBack', [], 'minecraft:nether_star')
        .addButton(50, 'Next page', [], 'minecraft:arrow')
        .addButton(48, 'Previous page', [], 'minecraft:arrow')
        .addPattren('bottom', '', [], 'textures/blocks/glass_black', [48, 49, 50])
        ...
        form.force(sender, (res) => {
            if (res.canceled) return
            if (res.selection == 49) shop(sender, 1)
            else if (res.selection == 48) shop(sender, page > 1 ? page - 1 : page, category)
            else if (res.selection == 50) shop(sender, page < pages ? page + 1 : page, category)
...
const manageCategories = (sender, page) => {
    const keys = Databases.shopCategories.keys(), pages = Math.ceil(keys.length / 45), categories = keys.slice((page - 1) * 45, (page - 1) * 45 + 45)
    const form = new CX.chestForm('large')
    .setTitle(`Manage Categories §f${page}§r/§f${pages}`)
    .addButton(49, '§cBack', [], 'minecraft:nether_star')
    .addButton(50, 'Next page', [], 'minecraft:arrow')
    .addButton(48, 'Previous page', [], 'minecraft:arrow')
    .addPattren('bottom', '', [], 'textures/blocks/glass_black', [48, 49, 50])
    ...
    form.force(sender, (res) => {
        if (res.canceled) return
        if (res.selection == 49) menu(sender)
        else if (res.selection == 48) manageCategories(sender, page > 1 ? page - 1 : page)
        else if (res.selection == 50) manageCategories(sender, page < pages ? page + 1 : page)
